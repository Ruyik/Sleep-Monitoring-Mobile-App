...
class SleepApp(App):
    ...
    def get_path(self, filename):
        """Joins the current directory with the filename safely for Android"""
    ...
    def build(self):
        ...
        # Loading KV files using safe path handler
        styles_path = self.get_path("UI/styles.kv")
        main_kv_path = self.get_path("UI/main.kv")
        ...
        screens_to_add = [
            ...
            'ConsistencyScreen', 'BotScreen', 'DashboardScreen', 'PetStatusScreen',
        ...
        Clock.schedule_once(lambda dt: self.switch('intro'), 0)
    ...
    def login_success(self, result):
        ...
        if result.startswith("✅"):
            ...
            # 2. Connect MQTT in a background thread so it doesn't freeze the app
            threading.Thread(target=self.dashboard_client.connect, daemon=True).start()
        else:
            self.set_status(screen, result)
